...
const PrismaticBurst = ({
  ...
  const dpr = Math.min(window.devicePixelRatio || 1, isMobile ? 1 : 2);
    ...
    if ('ResizeObserver' in window) {
      ...
      (window as Window).addEventListener('resize', resize as EventListenerOrEventListenerObject);
    }
    resize();
  ...
  // animationType may be a string not present in the map; default to 'rotate'
  program.uniforms.uAnimType.value = animTypeMap[(animationType as 'rotate' | 'rotate3d' | 'hover') ?? 'rotate'];
