...
const Hyperspeed = ({
  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        // Use a capped devicePixelRatio for performance and crispness
        this.renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 2));
        this.composer = new EffectComposer(this.renderer);

        // Make sure the canvas fills the container via CSS so clientWidth/clientHeight are reliable
        this.renderer.domElement.style.width = '100%';
        this.renderer.domElement.style.height = '100%';
        this.renderer.domElement.style.display = 'block';
        container.append(this.renderer.domElement);

        // Bind resize handler so we can remove the exact same reference later
        this._onWindowResize = this.onWindowResize.bind(this);
        window.addEventListener('resize', this._onWindowResize);

        // Observe container size changes (covers initial layout / CSS changes)
        try {
          this.resizeObserver = new ResizeObserver(() => this._onWindowResize());
          this.resizeObserver.observe(container);
        } catch (e) {
          // ResizeObserver may not be available in older environments; fallback to window resize only
        }

  // Ensure initial sizing after DOM/CSS layout (renderer/canvas first)
  // If camera isn't ready yet, onWindowResize will still size the renderer and composer
  this.onWindowResize();
      ...
      onWindowResize() {
        ...
        // coerce to numbers and ensure minimum of 1
        const rawWidth = this.container.clientWidth || this.container.offsetWidth || window.innerWidth || 1;
        const rawHeight = this.container.clientHeight || this.container.offsetHeight || window.innerHeight || 1;
        const width = Math.max(1, Number(rawWidth));
        const height = Math.max(1, Number(rawHeight));

        // Update pixel ratio in case devicePixelRatio changed (e.g., zoom / display change)
        const dpr = Math.min(window.devicePixelRatio || 1, 2);
        this.renderer.setPixelRatio(dpr);

        // Set renderer drawing buffer size (don't change style since canvas css is 100%)
        this.renderer.setSize(Math.floor(width), Math.floor(height), false);

  // Safely update camera aspect and projection (camera may not exist yet during init)
  ...
  this.camera.updateProjectionMatrix();
        ...
        if (this.composer && typeof this.composer.setSize === 'function') {
          this.composer.setSize(Math.floor(width), Math.floor(height));
        }
      }
      ...
      dispose() {
        ...
        // remove the exact same listener reference we added
